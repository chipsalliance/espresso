...
char *argv[];
{
    ...
    switch (option_table[option].key) {
        ...
        case KEY_ESPRESSO:
            ...
            EXECUTE(error = verify(PLA->F, Fold, PLA->D), VERIFY_TIME, PLA->F,
                    cost);
            if (error) {
                print_solution = FALSE;
                PLA->F = Fold;
        ...
        case KEY_MANY_ESPRESSO: {
            int pla_type;
            do {
                EXEC(PLA->F = espresso(PLA->F, PLA->D, PLA->R), "ESPRESSO   ",
                     PLA->F);
                if (print_solution) {
        ...
        case KEY_expand: /* execute expand */
            EXECUTE(PLA->F = expand(PLA->F, PLA->R, FALSE), EXPAND_TIME, PLA->F,
                    cost);
            break;
        ...
        case KEY_essen: /* check for essential primes */
            ...
            EXECUTE(F = essential(&(PLA->F), &(PLA->D)), ESSEN_TIME, PLA->F,
                    cost);
            free_cover(F);
            break;
        ...
        case KEY_test: {
            ...
            E = new_cover(10);
            sf_free(PLA->F);
            EXECUTE(PLA->F = complement(cube1list(T)), COMPL_TIME, PLA->F,
                    cost);
            EXECUTE(PLA->F = expand(PLA->F, T, FALSE), EXPAND_TIME, PLA->F,
                    cost);
            EXECUTE(PLA->F = irredundant(PLA->F, E), IRRED_TIME, PLA->F, cost);
            sf_free(T);
            T = sf_join(PLA->F, PLA->R);
            EXECUTE(PLA->D = expand(PLA->D, T, FALSE), EXPAND_TIME, PLA->D,
                    cost);
    ...
    *option = 0;
    for (i = 1; i < (*argc) - 1; i++) {
        if (strcmp(argv[i], "-do") == 0) {
            ...
            if (option_table[j].name == 0) {
                fprintf(stderr, "espresso: bad keyword \"%s\" following -do\n",
                        argv[i + 1]);
                exit(1);
            }
            break;
