...
map_output_symbolic(PLA) pPLA PLA;
{
    ...
    symbolic_hack_labels(PLA, PLA->symbolic_output, compress, cube.size,
                         old_size, tot_size);
    set_free(compress);
}
...
int verbose_mode;
{
    ...
    if (cube.num_vars - cube.num_binary_vars != 2) {
        fprintf(stderr, "use .symbolic and .symbolic-output to specify\n");
